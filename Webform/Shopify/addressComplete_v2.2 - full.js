...
window.getStore = function(){
    ...
    //var url = window.location.href; // uncomment when production
    var url = "https://us.kobobooks.com/";// comment when production

    // check store
    // buggy
     // var _country_codes = Object.keys(store);
     // for (var i = 0; i < _country_codes.length; i++){
       //   if (store[_country_codes[i]].pattern.test(url)){
     //         console.log(_country_codes[i]);
       //       return _country_codes[i];
     //     }
    //  }
...
function validateMaxLength(e) {
    ...
    if (currentLength > Max_Length) {
        ...
        address1.parentNode.className = "field__input-wrapper field field--required field--error field--focus";
        ...
        address1 = document.getElementById("checkout_shipping_address_address1");
        address1.parentNode.className = "field__input-wrapper field field--required";
...
if (getStore() == store.UnitedStates) {//The PO Box verification is need it only for the US store
    ...
    text.onkeyup = function(a) {
        ...
        for (i = 0; i < poBoxText.length; i++)
            ...
            1 == showText ? (text.parentNode.className = "field__input-wrapper field field--required field--error field--focus", message.style.display = "block") : (text.parentNode.className = show ? "field__input-wrapper field field--required field--error field--focus" : "field__input-wrapper field field--required field--focus", message.style.display = "none")
        };
        for (i = 0; i < poBoxText.length; i++)
            if (-1 != text.value.toLowerCase().indexOf(poBoxText[i].toLowerCase())) {
                showText = !0;
